const STORAGE_KEY = "soil-health-report-nagaland";
  ...
  return `
    <div class="soil-card">
      <header class="soil-card-header">
        <div class="soil-card-header-content">
          <div class="soil-card-title">
            <h1>Soil Health Report (Independent Service)</h1>
            <h2>Independent interpretation of soil test data — not an official government Soil Health</h2>
          </div>
        </div>
      </header>
      ...
      <section class="soil-card-section">
        <h4>Disclaimer</h4>
        <div class="recommendation-box">
          <p>This report is generated by the Department of Soil & Water Conservation, Nagaland for research and training. It is NOT the legally recognised Government of India Soil Health Card. For the official and legally valid Soil Health, please visit <a href="https://soilhealth.dac.gov.in" target="_blank" rel="noopener">soilhealth.dac.gov.in</a>. Use this report only as advisory information and only after obtaining farmer consent to process their data.</p>
        </div>
      </section>
...
function renderDistrictDashboard() {
  ...
  ui.dashboardSubtitle.textContent = "Enter soil test data and generate Soil Health Reports for the district.";
  ...
  const html = `
    ...
    <html lang="en">
      <head>
        ...
        <title>${card.id} - Soil Health Report</title>
...
ui.previewButton.addEventListener("click", () => {
  ...
  // Preview does not persist, but require confirmation that farmer consent is present
  const consent = confirm("Confirm: you have the farmer's explicit consent to process and preview their soil data?");
  if (!consent) {
    setMessage(ui.soilCardMessage, "Consent is required to preview soil data.", "error");
    return;
  }
...
ui.soilCardForm.addEventListener("submit", (event) => {
  ...
  // Ensure explicit farmer consent is confirmed before saving/storing personal soil data
  ...
  if (!consent) {
    setMessage(ui.soilCardMessage, "Consent is required to save this soil report.", "error");
    return;
  }
  ...
  lastCardForPdf = card;
  setMessage(ui.soilCardMessage, `Soil Health Report ${card.id} saved successfully.`, "success");
});

ui.adminDownloadPdfButton?.addEventListener("click", () => {
  if (!lastCardForPdf) {
    alert("Please select a Soil Health Report first.");
    return;
  }
  // Confirm consent before allowing PDF download
...
ui.districtDownloadPdfButton?.addEventListener("click", () => {
  if (!lastCardForPdf) {
    alert("Please preview or save a Soil Health Report first.");
    return;
  }
  const consent = confirm("Confirm: you have the farmer's explicit consent to download/print their soil report? (Required)");
  if (!consent) return;
...
document.addEventListener("click", (event) => {
  ...
  if (deleteCardButton) {
    ...
    const confirmed = confirm(`Delete Soil Health Report ${card.id}?`);
    if (!confirmed) return;
    ...
    if (currentUser?.role === "admin") {
      setMessage(ui.districtAccountMessage, `Deleted Soil Health Report ${card.id}.`, "success");
    } else {
      setMessage(ui.soilCardMessage, `Deleted Soil Health Report ${card.id}.`, "success");
    }
    return;
  }
});
